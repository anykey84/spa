...
appMain.directive('myPersonDocument', function () {
    return {
        restrict: 'E',
        replace: true,
        templateUrl: '/Template/PersonDocument',
        scope: {
            typedoc: '=',
            visibleadddocument: '=',
            visiblefiledocument: '=',
        ...
        controller: (['$scope', '$routeParams', 'personDocumentService', 'personFileService', 'Person', 'Office', 'Profile',
           function ($scope, $routeParams, personDocumentService, personFileService, Person, Office, Profile, ngMask) {
               ...
               $scope.isNew = angular.isUndefined($routeParams.par);
                           ...
                           if (result.data.length) {
                               ...
                               for (var prop in values) {
                                   if (values[prop]['propertyType'] === 'date') {
                                       ...
                                       values[prop]['propertyValue'] = formatDate(dt);
                                   }
                               }
                           }
               ...
               $scope.initpdp = function (item) {
                   ...
                   if (!angular.isUndefinedOrNull($scope.lastProperty) && item.id_Person_DocumentProperty == $scope.lastProperty.id_Person_DocumentProperty) {
               ...
               $scope.documentPropertyChanged = function (item) {
                   ...
                   //if (angular.isUndefined($scope.lastProperty)) {
                   //    $scope.lastProperty = {};
                   //    angular.copy(item, $scope.lastProperty);
                   //    console.log('произошла переинициализация');
                   //    return;
               ...
               $scope.documentPropertyChildChanged = function (item) {
                   ...
                   if (angular.isUndefined($scope.lastChildProperty)) {
                       $scope.lastChildProperty = {};
                       //console.log(item.id_Person_DocumentProperty);
               ...
               $scope.validatePerson_DocumentProperty = function (item) {
                   ...
                   if (angular.isUndefinedOrNull(item.messages))
                       item.messages = []
                   else
                   ...
                   personDocumentService
                       ...
                       .then(function (result) {
                           ...
                           if ((!angular.isUndefinedOrNull(result.data)) && (result.data.length > 0)) {
