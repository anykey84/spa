export default (ngModule) => {
    ...
    ngModule.directive('myCashRests', function () {
        return {
            restrict: 'EA',
            replace: true,
            template: require('./CashRests.html'),
            scope: {},
            controller: (['$scope', 'ticketService', 'Office', '$filter',
                function ($scope, ticketService, Office, $filter) {
                    ...
                    {   
                        if (checkDate == "" || _.isNil(checkDate)) {                        
                            return false;
                        }
                        else {
                            return true;
                    ...
                    $scope.restsTableVisible = function()
                    {
                        if($scope.processing['loadCashRests'])
                            return false;
                        if(_.isNil($scope.rests))
                            return false;
                        if(_.isNil($scope.rests.length))
                            return false;
