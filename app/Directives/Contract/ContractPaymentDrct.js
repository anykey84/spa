export default (ngModule) => {
    ...
    ngModule.directive('myContractpayment', function () {
        return {
            restrict: 'E',
            replace: true,
            template: require('./ContractPayment.html'),
            scope: {
                contract: '='
            },
            controller: (['$scope', '$routeParams', 'personFileService', 'reportService', 'paymentService', 'Person', 'Office', 'Profile', 'officeService',
               function ($scope, $routeParams, personFileService, reportService, paymentService, Person, Office, Profile, officeService) {

                   $scope.showFormAddPko = false;
                   $scope.showFormAddRko = false;
                   $scope.showFormAddBankRko = false;
                   ...
                   $scope.addPayment = function (typePayment) {
                       ...
                       officeService.getCurrentOffice()
                         .then(function (result) {
                             $scope.payment.idOffice = result.data.id_Office;

                             if ($scope.payment.typePayment === 'rkobank') {
                       ...
                       paymentService
                           .getPayment($scope.contract.id)
                               .then(function (result) {
                                   ...
                                   $scope.btnPKO_show = $scope.payments.some(function (item) { return item.ispko === false; });
                                   $scope.btnRKO_show = !$scope.btnPKO_show;
